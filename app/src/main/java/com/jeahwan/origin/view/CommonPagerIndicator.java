...
public class CommonPagerIndicator extends View implements IPagerIndicator {
    ...
    public static final int MODE_DRAWABLE_RIGHT_BOTTOM = 3; // drawable宽高都为icon宽高，必须为图片资源，展示在右下角
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        ...
        } else if (mMode == MODE_DRAWABLE_RIGHT_BOTTOM) {
            leftX = current.mLeft + current.width() - mIndicatorDrawable.getMinimumWidth() + mXOffset;
            nextLeftX = next.mLeft + next.width() - mIndicatorDrawable.getMinimumWidth() + mXOffset;
            rightX = current.mLeft + current.width() - mXOffset;
            nextRightX = next.mLeft + next.width() - mXOffset;
            mDrawableRect.top = getHeight() - mIndicatorDrawable.getMinimumHeight();
            mDrawableRect.bottom = (int) (getHeight() - mYOffset);
        } else {    // MODE_EXACTLY
            leftX = current.mLeft + (current.width() - mIndicatorDrawable.getMinimumWidth()) / 2f;
            nextLeftX = next.mLeft + (next.width() - mIndicatorDrawable.getMinimumWidth()) / 2f;
            rightX = current.mLeft + (current.width() + mIndicatorDrawable.getMinimumWidth()) / 2f;
            nextRightX = next.mLeft + (next.width() + mIndicatorDrawable.getMinimumWidth()) / 2f;
            mDrawableRect.top = (int) (getHeight() - mIndicatorDrawable.getMinimumHeight() - mYOffset);
            mDrawableRect.bottom = (int) (getHeight() - mYOffset);
        }
    ...
    public void setMode(int mode) {
        if (mode == MODE_EXACTLY || mode == MODE_MATCH_EDGE || mode == MODE_WRAP_CONTENT || mode == MODE_DRAWABLE_RIGHT_BOTTOM) {
            mMode = mode;
        } else {
